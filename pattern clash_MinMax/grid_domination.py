...
################################################################
    def check_dig_minmax(self,x,y,code):
      counter=0
      if x+2 <5 and y+2 <5:
        if (self.grid[x+2][y+2]==code or self.grid[x+2][y+2]=='.') and (self.grid[x+1][y+1]==code or self.grid[x+1][y+1]=='.'):
          counter+=1
      if x+2<5 and y-2>=0:
        if (self.grid[x+2][y-2]==code or self.grid[x+2][y-2]=='.') and (self.grid[x+1][y-1]==code or self.grid[x+1][y-1]=='.'):
          counter+=1
...
################################################################
    def check_cons_minmax (self,x,y,code):
        counter=0
        if x+2 < 5:
          if (self.grid[x+2][y]==code or self.grid[x+2][y]=='.') and (self.grid[x+1][y]=='.') :
             counter+=1
        if y+2 < 5:
          if (self.grid[x][y+2]==code or self.grid[x][y+2]== '.') and (self.grid[x][y+1]==code or self.grid[x][y+1]=='.' ):
            counter+=1
        return counter
################################################################
    def squ_minmax(self, row, col, code):
      counter = 0
      square_shape = [
              [(0, 0), (1, 0), (0, 1),(1,1)]
          ]
...
################################################################
    def min(self,depth,grid):
        depth -= 1
        if depth < 0:
            return
        
    def max(self,depth,grid):
        depth -= 1
        if depth < 0:
            return
